...
- **精确 7 天日均** — 通过 API 直接查询近 7 天请求总数，首次安装即可获得精确日均值，无需等待本地数据积累
- **智能预测** — 套餐充裕时直接显示"本周期充裕"；套餐耗尽后基于按量计费的实际花费速率预测剩余天数
- **智能预警** — 可配置阈值（默认 80%），触发背景色闪烁 + 弹窗提醒，同一周期仅提醒一次
- **MAX 模式监测** — 实时检测 MAX 模式、Thinking 模式等高消耗状态，状态栏醒目提示 + 切换至 MAX 时弹窗告警
...
- **数据源**: `cursor.com/api/usage` + `/api/usage-summary` + `/api/dashboard/get-filtered-usage-events`（今日 & 近 7 天请求计数）
...
- **日均计算**: 三级策略 — ① API 返回的精确 7 天请求总数 ÷ 7 → ② 本地快照差值 → ③ 周期累计 ÷ 已过天数
- **预测**: 套餐未耗尽 → 显示"充裕"；套餐耗尽 → 推算按量计费已用天数 → 日均花费 = 按量总支出 ÷ 已用天数 → 剩余预算 ÷ 日均花费
